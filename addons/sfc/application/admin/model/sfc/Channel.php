...
class Channel extends Model
{
    ...
    // 表名
    protected $name = 'sfc_test_channel';
    // 自动写入时间戳字段
    ...
    protected static function init()
    {
        ...
        self::afterDelete(function ($row) {
            ...
            $childIds[] = $row['id'];
            db('sfc_test_archives')->where('channel_id', 'in', $childIds)->update(['deletetime' => time()]);
        });
        self::afterWrite(function ($row) {
            ...
            if (isset($changed['status']) && $changed['status'] == 'hidden') {
                ...
                $childIds = $tree->getChildrenIds($row['id']);
                db('sfc_test_channel')->where('id', 'in', $childIds)->update(['status' => 'hidden']);
            }
        });
    }
