...
use app\admin\model\sfc\Channel;
use app\admin\model\sfc\ChannelAdmin;
use app\admin\model\sfc\Modelx;
...
class Archives extends Backend
{
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\sfc\Archives;
        ...
        $sfc = get_addon_config('sfc');
        $this->assignconfig('sfc', ['archiveseditmode' => $sfc['archiveseditmode']]);
    ...
    public function index()
    {
        ...
        $modelList = \app\admin\model\sfc\Modelx::all();
    ...
    {
        $model = \app\admin\model\sfc\Modelx::get($model_id);
        if (!$model) {
            $this->error('未找到对应模型');
        }
        $fieldsList = \app\admin\model\sfc\Fields::where('model_id', $model['id'])->where('type', '<>', 'text')->select();
        ...
        if ($this->request->isAjax()) {
            ...
            $total = Db::table($table)
                ->alias('main')
                ->join('sfc_test_channel channel', 'channel.id=main.channel_id', 'LEFT')
    ...
    public function edit($ids = NULL)
    {
        ...
        $model = \app\admin\model\sfc\Modelx::get($channel['model_id']);
        if (!$model) {
    ...
    {
        \app\admin\model\sfc\Archives::event('after_delete', function ($row) {
    ...
    public function move($ids = "")
    {
        if ($ids) {
            ...
            if ($channel && $channel['type'] === 'list') {
                $channelNums = \app\admin\model\sfc\Archives::
                with('channel')
    ...
    public function get_channel_fields()
    {
        ...
        if ($channel && $channel['type'] === 'list') {
            ...
            $fields = \app\admin\model\sfc\Fields::where('model_id', $channel['model_id'])
