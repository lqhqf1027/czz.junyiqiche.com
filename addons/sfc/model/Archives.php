...
class Archives Extends Model
{
    ...
    public function getContentAttr($value, $data)
    {
        ...
        if (preg_match($pattern, $value) && !$this->getAttr('ispay')) {
            $payurl = addon_url('sfc/order/submit', ['id' => $data['id']]);
    ...
    public function getTagslistAttr($value, $data)
    {
        ...
        foreach (array_filter(explode(",", $data['tags'])) as $k => $v) {
            $list[] = ['name' => $v, 'url' => addon_url('sfc/tags/index', [':name' => $v])];
        }
        return $list;
    }
    ...
    public function getUrlAttr($value, $data)
    {
        ...
        return addon_url('sfc/archives/index', [':id' => $data['id'], ':diyname' => $diyname, ':channel' => $data['channel_id']]);
    }

    public function getFullUrlAttr($value, $data)
    {
        $diyname = $data['diyname'] ? $data['diyname'] : $data['id'];
        return addon_url('sfc/archives/index', [':id' => $data['id'], ':diyname' => $diyname, ':channel' => $data['channel_id']], true, true);
    }
