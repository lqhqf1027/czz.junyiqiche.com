...
class Comment Extends Model
{
    ...
    public static function postComment($params = [])
    {
        ...
        if (isset($params['pid'])) {
            ...
            if ($parentComment && $parentComment['subscribe'] && Validate::is($parentComment->user->email, 'email')) {
                ...
                $unsubscribe_url = addon_url("sfc/comment/unsubscribe", ['id' => $parentComment['id'], 'key' => md5($parentComment['id'] . $parentComment->user->email)], true, true);
    ...
    public static function getCommentList($params)
    {
        ...
        $list = self::with('user')
            ...
            ->order($order)
            ->cache($cache)
            ->paginate($pagesize, false, ['type' => '\\addons\\sfc\\library\\Bootstrap', 'var_page' => 'cp', 'fragment' => $fragment]);
        self::render($list);
        return $list;
    }
    ...
    {
        return $this->belongsTo("addons\sfc\model\Archives", 'aid')->field('id,title,image,diyname,model_id,channel_id,likes,dislikes,tags,createtime')->setEagerlyType(1);
    }
